    ...
    async def run(self, context: AgentContext) -> AgentResult:
        ...
        for poller in pollers:
            try:
                ...
                stats["errors"] += 1
                if context.logger:
                    context.logger.error("source.poll.failed", source=poller.name, error=str(e))

        # Merge into shared state for analysis pipeline (don't overwrite — analysis may not have consumed previous docs yet)
